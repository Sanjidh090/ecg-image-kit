...
the estimated grid size in pixels for both horizontal and vertical directions.
...
Outputs:
    ...
    fine_grid_res - Estimated fine grid size (in pixels), representing 40ms in time and 0.1mV in amplitude.
...
Example:
    ...
    # Estimate grid size
    coarse_grid_res, fine_grid_res = ecg_grid_size_from_paper(img, paper_width[0], unit)
...
def ecg_grid_size_from_paper(img, paper_width, unit):
    """
    ECG grid size estimate from paper size and image width
    ...
    paper_width : float
        The paper width, corresponding to the second dimension of the input image
    unit : str
        A string specifying the unit of paper_width ('cm' or 'in')
    
    Returns
    -------
    coarse_grid_res : float
        Estimated coarse grid size (in pixels), representing 0.2s in time and 0.5mV in amplitude
    fine_grid_res : float
        Estimated fine grid size (in pixels), representing 40ms in time and 0.1mV in amplitude
    """
    width = img.shape[1]
    ...
    # Standard fine ECG grid 1mm x 1mm (0.1mV x 40ms)
